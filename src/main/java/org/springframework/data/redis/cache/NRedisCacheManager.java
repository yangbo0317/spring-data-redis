...
public class NRedisCacheManager extends AbstractTransactionSupportingCacheManager {
	...
	/**
	 * Creates new {@link NRedisCacheManager} using given {@link RedisCacheWriter} and default
	 ...
	 * @return new instance of {@link NRedisCacheManager}.
	 */
	public static NRedisCacheManager defaultCacheManager(RedisConnectionFactory connectionFactory) {
	...
	/**
	 * Entry point for builder style {@link NRedisCacheManager} configuration.
	 *
	 * @param connectionFactory must not be {@literal null}.
	...
	@Override
	protected NRedisCache getMissingCache(String name) {
	...
	public Map<String, RedisCacheConfiguration> getCacheConfigurations() {
		...
		getCacheNames().forEach(it -> {
			...
			NRedisCache cache = NRedisCache.class.cast(lookupCache(it));
	...
	/**
	 * Configuration hook for creating {@link NRedisCache} with given name and cacheConfig.
	 *
	 * @param name must not be {@literal null}.
	 * @param cacheConfig can be {@literal null}.
	 * @return never {@literal null}.
	 */
	protected NRedisCache createRedisCache(String name, @Nullable RedisCacheConfiguration cacheConfig) {
		return new NRedisCache(name, cacheWriter, cacheConfig != null ? cacheConfig : defaultCacheConfig);
	}

	/**
	 * Configurator for creating {@link NRedisCacheManager}.
	 *
	 * @author Christoph Strobl
	 * @since 2.0
	 */
	public static class RedisCacheManagerConfigurator {
		...
		/**
		 * Define a default {@link RedisCacheConfiguration} applied to dynamically created {@link NRedisCache}s.
		 *
		...
		/**
		 * Enable {@link NRedisCache}s to synchronize cache put/evict operations with ongoing Spring-managed transactions.
		 *
		 * @return new instance of {@link RedisCacheManagerConfigurator}.
		 */
		...
		/**
		 * Create new instance of {@link NRedisCacheManager} with configuration options applied.
		 *
		 * @return new instance of {@link NRedisCacheManager}.
		 */
		public NRedisCacheManager build() {

			NRedisCacheManager cm = new NRedisCacheManager(cacheWriter, defaultCacheConfiguration, intialCaches) {
